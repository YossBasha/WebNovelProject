async function renderNovelDetails() {
  ...
  if (chipsEl) {
    chipsEl.innerHTML = novel.categories.map(catId => {
  ...
  if (priceEl) {
    priceEl.textContent = novel.price ? `$${parseFloat(novel.price).toFixed(2)}` : "Free";
  }
...
async function renderRelatedNovels(currentNovel, allCategories) {
  ...
  let allNovels = [];
  try { allNovels = await fetchNovels(); } catch (_) { return; }
  ...
  if (related.length > 0) {
    slider.innerHTML = related.map(n => createNovelCard(n, allCategories)).join("");
  } else {
...
function initRatingSystem() {
  ...
  stars.forEach(star => {
    ...
    star.addEventListener("click", () => { selectedRating = parseInt(star.dataset.rating); highlightStars(selectedRating); });
  });
